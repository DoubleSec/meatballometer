...
class WainoNet(nn.Module):
    ...
    def forward(self, x, pad_mask, pretrain_mask):
        # I'm back to thinking this is wrong.

        pretrain_mask = pretrain_mask.tile([pretrain_mask.shape[-1]]).reshape(
            [-1, pretrain_mask.shape[-1], pretrain_mask.shape[-1]]
        )
        # This needs to be repeat_interleave, and not tile, it turns out.
        pretrain_mask = pretrain_mask.repeat_interleave(self.n_head, dim=0)

        # pad_mask = pretrain_mask & pad_mask

        x = self.pitch_embedding(x)
        x = self.position_encoding(x)
        x = self.tr(x, src_key_padding_mask=pad_mask, mask=pretrain_mask)
...
class Waino(pl.LightningModule):
    ...
    def forward(self, x, pad_map, pretrain_map):
        # Literally this only gets used to make a graph, wtf

        return self.net(x, pad_map, pretrain_map)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.mask_tokens:
            ...
            y_hat = self.net(x2, pad_mask, pretrain_mask)
        else:
            y_hat = self.net(x, pad_mask, pretrain_mask)
